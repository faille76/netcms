...
final class AdminDeleteImageAlbumController extends AbstractController
{
    use UserTrait;
    ...
    /**
     * @Route("/admin/gallery/albums/{albumId}/pictures/{pictureId}/delete", name="admin_gallery_album_pictures_delete", methods={"GET"})
    ...
    public function __invoke(Request $request, int $albumId, int $pictureId): Response
    {
        ...
        $this->commandBus->dispatch(new DeleteImageCommand(
            $albumId,
            $pictureId,
            $this->getUserId(),
            time()
